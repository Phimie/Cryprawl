    ...
    def __init__(self, sf_game):
        ...
        for i in range(1, 5):
            self.frames_move.append(pygame.transform.smoothscale(
                pygame.image.load(rpath.rpath(f'assets/images/bat/{i}.png')).convert_alpha(), 
                (64, 64)
            ))
        self.frames_hitted.append(pygame.transform.smoothscale(
            pygame.image.load(rpath.rpath('assets/images/bat/hitted.png')).convert_alpha(), 
            (64, 64)
        ))
        for i in range(1, 11):
            self.frames_death.append(pygame.transform.smoothscale(
                pygame.image.load(rpath.rpath(f'assets/images/bat/death/{i}.png')).convert_alpha(), 
                (64, 64)
            ))
        ...
        self.spawn_xy = random.choice([
            ...
            [self.screen_rect.centerx + 220, self.screen_rect.centery - 290],
            [self.screen_rect.centerx - 300, self.screen_rect.centery - 290]
        ])
